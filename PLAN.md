...
_No active wave._ Pick from **Deferred** only when it strengthens the governance thesis.

Suggested later (not started):

- Optional Grafana JSON dashboard (visual polish on existing `/metrics`)
- One second vertical as a second harness file (“same CI, different agent”) — only after this flagship stays crisp
- Prompt-regression gate when a golden set exists
...
| Prompt regression + token budget gates | Add when suites exist |
